...
def _plot_hydrograph_svg(
    ...
    all_series = list(simulations.values())
    if observed is not None:
    ...
    for idx, (model_id, series) in enumerate(simulations.items()):
    ...
    legend_spacing = 90
    for idx, model_id in enumerate(simulations.keys()):
        color = colors[idx % len(colors)]
    ...
    if observed is not None:
        obs_x = legend_x + len(simulations) * legend_spacing
        parts.append(

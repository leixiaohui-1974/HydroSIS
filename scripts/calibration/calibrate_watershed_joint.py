...
⚠️ 注意：这是一个高级研究工具脚本
本脚本实现了24维全局优化问题，计算成本高，适用于研究场景。
普通用户请使用cascading或hybrid策略。

参考: scripts/calibration/README_ADVANCED_SCRIPTS.md

核心改进：
1. 同时优化所有6个分区的参数（24维优化问题）
2. 使用多个控制点的观测数据（不只是出口）
3. 多目标优化：平衡各控制点的NSE
4. 空间正则化：相邻分区参数保持相似性
5. 全局优化：避免局部最优
...
WATERSHED_TOPOLOGY = {
    ...
    'control_points': {
        'P3': {'observe': True, 'weight': 1.0},   # 上游控制点
        'P6': {'observe': True, 'weight': 1.0},   # 中上游控制点
        'P2': {'observe': True, 'weight': 1.0},   # 中游控制点
        'P5': {'observe': True, 'weight': 1.0},   # 支流控制点
        'P4': {'observe': True, 'weight': 1.5},   # 中下游控制点（权重更大）
        'P1': {'observe': True, 'weight': 2.0},   # 出口控制点（权重最大）
    },
}
...
def simulate_watershed(
    ...
    参数：
        rainfall: 降雨序列
        zone_params: 每个分区的HBV参数 {zone_id: params}

    返回：
        每个分区的本地产流和出口流量 {zone_id: {'local': ..., 'outlet': ...}}
    """
...
class WatershedCalibrator:
    ...
    def __init__(
        ...
        参数：
            rainfall: 降雨序列
            observations: 各控制点的观测数据 {zone_id: observed_runoff}
            control_points: 控制点配置 {zone_id: {'observe': bool, 'weight': float}}
        """
        ...
        # 定义参数空间：每个分区4个参数
        # [field_capacity, beta, k0, k1] × 6分区 = 24参数
        ...
        print(f"  参数维度: {self.n_params_total} ({self.n_zones}分区 × {self.n_params_per_zone}参数)")
        print(f"  控制点数: {len([z for z, c in control_points.items() if c['observe']])}")
    ...
    def objective(self, x: np.ndarray) -> float:
        ...
        # 总目标：最大化NSE，即最小化 -NSE + penalty
        objective_value = -avg_nse + spatial_penalty

        return objective_value

    def calibrate(self, maxiter: int = 30, popsize: int = 15) -> Dict:
        """
        执行联合率定
...
def main():
    ...
    output_dir = Path('results/watershed_calibration_joint')
    ...
    for zone_id in WATERSHED_TOPOLOGY['zones']:
        ...
        print(f"  {zone_id}: 出口径流={obs['outlet'].sum():.1f} mm, "
              f"本地产流={obs['true'].sum():.1f} mm, "
              f"上游入流={obs['upstream_inflow'].sum():.1f} mm")
    ...
    calibrator = WatershedCalibrator(
        rainfall=rainfall,
        observations=observations,
        control_points=WATERSHED_TOPOLOGY['control_points'],
    )

    calibration_result = calibrator.calibrate(maxiter=30, popsize=15)
    ...
    summary_path = output_dir / 'joint_calibration_summary.csv'
    ...
    for zone_id in WATERSHED_TOPOLOGY['zones']:
        ...
        axes[0].bar(timestamps, rainfall, width=0.04, alpha=0.6)
        axes[0].set_ylabel('Rainfall (mm/h)')
        axes[0].set_title(f'{zone_id} - Joint Calibration')

        perf = calibration_result['performance'][zone_id]
        axes[1].plot(timestamps, obs['outlet'], 'k-', label='Observed', linewidth=1.5)
        axes[1].plot(timestamps, result['outlet'], 'r--', label='Simulated (Joint)', linewidth=1.5)
        axes[1].set_ylabel('Outlet Flow (mm/h)')
        axes[1].legend()
        axes[1].set_title(f'NSE = {perf["NSE"]:.4f}')

        axes[2].plot(timestamps, result['local'], 'g-', label='Local Runoff', linewidth=1.5)
        axes[2].plot(timestamps, result['upstream'], 'm-', label='Upstream Inflow', linewidth=1.5, alpha=0.7)
        axes[2].set_ylabel('Flow (mm/h)')
        axes[2].set_xlabel('Time')
        axes[2].legend()

        plt.tight_layout()
        fig_path = zone_dir / 'plot.png'
        plt.savefig(fig_path, dpi=150, bbox_inches='tight')
        plt.close()

    print(f"\n✓ 联合率定完成！")
    print(f"  平均NSE: {summary_df['NSE'].mean():.4f}")
    print(f"  出口P1 NSE: {summary_df[summary_df['zone']=='P1']['NSE'].values[0]:.4f}")
    print(f"  输出目录: {output_dir}")

    return summary_df, calibration_result


if __name__ == '__main__':
    summary_df, result = main()

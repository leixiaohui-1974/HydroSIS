...
# 待率定参数的搜索范围（调整以匹配简单径流系数模型的响应）
# 策略：让HBV尽可能表现得像线性模型
param_bounds = {
    'FC': [200, 600],       # 增大土壤容量（减少饱和产流）
    'BETA': [1.0, 3.0],     # 降低BETA（更接近线性）
    'K0': [0.08, 0.4],      # 提高K0下限（加快响应）
    'K1': [0.02, 0.15],     # 中等K1范围
    'K2': [0.005, 0.05],    # 提高K2下限（保证基流）
    'PERC': [0.5, 5.0],     # 降低PERC上限（减少渗漏）
    'initial_soil_ratio': [0.6, 0.95],  # 初始土壤湿度比例（FC的百分比）
    'initial_upper': [20, 100],         # 提高初始上层储量（立即响应）
}
...
def run_hbv_model(FC, BETA, K0, K1, K2, PERC, initial_soil_ratio, initial_upper):
    ...
    Args:
        ...
        K2: 基流消退系数
        PERC: 渗漏率 (mm/h)
        initial_soil_ratio: 初始土壤湿度比例（FC的百分比）
        initial_upper: 初始上层储量 (mm)
...
def calibration_objective(FC, BETA, K0, K1, K2, PERC, initial_soil_ratio, initial_upper):
    """率定目标函数：最大化NSE
...
calibration_configs = {
    'SCE-UA': {
        'method': 'sce_ua',
        'n_complexes': 6,  # 增加complex数（适应8参数）
        'max_iterations': 150,  # 8参数需要更多迭代
        'patience': 30,
    },
    'PSO': {
        'method': 'pso',
        'n_particles': 50,  # 8参数需要更多粒子
        'max_iterations': 150,
        'w': 0.7,
        'c1': 1.5,
        'c2': 1.5,
